...
/**
 * Resolves the raw sonaraem_invite cookie value to the waitlist signup it
 * belongs to — used to gate the Spotify OAuth redirect on an allowlist slot
 * before the user has an account (#372). Returns null for a malformed token,
 * one that doesn't match any row, or a row with no spotifyEmail on record
 * (pre-#290 signups never collected one).
 */
